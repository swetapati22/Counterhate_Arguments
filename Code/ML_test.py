...
def main():
    ...
    # Loading the raw test data CSV file
    test_df = pd.read_csv(os.path.join(args.data_dir, 'test_raw.csv'))
    
    # Adding a new column for predicted labels
    test_df['predicted_label'] = preds
    
    # Saving the updated DataFrame with predictions
    test_df.to_csv(os.path.join(args.output_dir, 'test_with_predictions.csv'), index=False)
